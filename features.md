...
### Total Feature Count by Configuration

| Configuration | Base | Aggregate | Historical | Total |
|--------------|------|-----------|------------|-------|
| **Minimal** | 28 | 0 | 0 | 28 |
| **Simple Baseline** | 28 | 6 (location/state only) | 0 | 34 |
| **Recommended** | 28 | 122 (full cascade metadata) | 0 | 150 |
| **Full** | 28 | 122 | 20 | 170 |

### Expected Performance

| Configuration | Features | Training Time | Expected F1 (Macro) |
|--------------|----------|---------------|---------------------|
| Minimal | 28 | ~30 sec | 0.35-0.40 |
| Simple Baseline | 34 | ~1 min | 0.38-0.43 |
| **Recommended** | **150** | **~2 min** | **0.45-0.55** |
| Full | 170 | ~8 min | 0.50-0.65 |
...
### Required Data Files
- `events_labeled.csv` - Main event data with all raw features
- `labels_binary.csv` - Multilabel targets (29 binary columns)
- `cascade_pairs.csv` - For aggregate features (optional but recommended)

### Preventing Data Leakage
- Use `AggregateFeatureTransformer` to fit on training data only
- Apply same learned statistics to test data
- Never compute statistics from full dataset before splitting

### Feature Selection Priority
1. **High priority**: severity_score, event_type_encoded, all cascade priors
2. **Medium priority**: location/state statistics, temporal features
3. **Low priority**: event_rarity_score, historical features
